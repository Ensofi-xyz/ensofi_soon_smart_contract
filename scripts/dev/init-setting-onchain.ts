...
(async () => {
  ...
  const usdcMint = new PublicKey(
    process.env.USDC_MINT_ADDRESS as string
  );
  ...
  //NOTE: Soon devnet currently used SOL as native mint, when go into mainnet will used ETH instead
  ...
  const solMint = new PublicKey(process.env.SOL_MINT_ADDRESS as string);
  const solPrice = 16845928611;
  const solExpo = -8;
  const solConf = 16767182;
  const SOLUSDPriceFeedName = process.env.SOL_USD_PRICE_FEED_NAME as string;

  const seedSOLPriceFeed = [
    Buffer.from("enso"),
    Buffer.from(SOLUSDPriceFeedName),
    program.programId.toBuffer(),
  ];

  const solPriceFeedAccount = PublicKey.findProgramAddressSync(
    seedSOLPriceFeed,
    program.programId
  )[0];
  ...
  await setPriceFeedAccount({
    ...
    expo: solExpo,
    price: solPrice,
    priceFeedId: SOLUSDPriceFeedName,
    program,
    ownerAccountSetting,
    connection,
  });
  ...
  const seedSOLAssetAccount = [
    Buffer.from("enso"),
    Buffer.from("asset"),
    solMint.toBuffer(),
    program.programId.toBuffer(),
  ];
  const solAssetAccount = PublicKey.findProgramAddressSync(
    seedSOLAssetAccount,
    program.programId
  )[0];
  ...
  await initAsset({
    ...
    isCollateral: true,
    priceFeedId: SOLUSDPriceFeedName,
    priceFeedAccount: solPriceFeedAccount,
    tokenMint: solMint,
    maxPriceAgeSeconds: 75,
    asset: solAssetAccount,
    ownerAccountSetting: ownerAccountSetting,
    connection,
    program,
  });
